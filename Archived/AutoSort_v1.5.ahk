...
checkErr(value) {
    if (InStr(value, "error") > 0 && InStr(value, "code") > 0 && InStr(value, "message") > 0 && InStr(value, "forbidden") > 0) {
        ...
        if (StrLen(newId) > 0) {
            IniWrite, %newId%, Config.ini, Id, poesessid
        }
...
jRequest() {
    IniRead, sessid, Config.ini, Id, poesessid
        
    HttpObj := ComObjCreate("WinHttp.WinHttpRequest.5.1")
    HttpObj.SetTimeouts(6000,6000,6000,6000) ;Set timeouts to 6 seconds

    ;ToolTip, Getting json entries
    url := "https://pathofexile.com/character-window/get-items?character=IHaveLottaMana"
